...
const toolsDeclarations: Tool[] = [
  {
    functionDeclarations: [
      ...
      },
      {
        ...
        description: 'Detailed configuration of the particle system.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            count: { type: Type.NUMBER, description: 'Number of particles (20000 - 60000)' },
            size: { type: Type.NUMBER, description: 'Size of particles (1 - 50)' },
            density: { type: Type.NUMBER, description: 'Density percentage (0 - 100)' }
          }
        }
      },
      {
        name: 'controlPhysics',
        description: 'Control the physics engine parameters.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            strength: { type: Type.NUMBER, description: 'Repulsion strength (0 - 100)' },
            radius: { type: Type.NUMBER, description: 'Repulsion radius (10 - 100)' }
          }
        }
      },
      {
        name: 'controlView',
        description: 'Control the overall view, UI visibility, and background.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            bgMode: { type: Type.STRING, enum: ['dark', 'light', 'gradient', 'auto'], description: 'Background mode' },
            uiHidden: { type: Type.BOOLEAN, description: 'True to hide UI (Clean Mode), False to show' },
            autoRotate: { type: Type.BOOLEAN, description: 'Enable/Disable camera auto-rotation' },
            screensaver: { type: Type.BOOLEAN, description: 'True to activate screensaver immediately' }
          }
        }
      },
      {
        ...
        parameters: {
          type: Type.OBJECT,
          properties: {
            bloom: { type: Type.BOOLEAN, description: 'Enable/Disable Neon Glow (Bloom)' },
            trails: { type: Type.BOOLEAN, description: 'Enable/Disable Motion Trails' },
            depth: { type: Type.NUMBER, description: '3D Depth Intensity (0 - 20)' },
            lyrics3D: { type: Type.BOOLEAN, description: 'Enable/Disable 3D Lyric Particles' }
          }
        }
      },
      {
...
const App: React.FC = () => {
          ...
          const sessionPromise = ai.live.connect({
              ...
              callbacks: {
                  onopen: () => {
                      ...
                      console.log("Gemini Live Connected - Full Control Enabled");
                      ...
                      if (msg.toolCall) {
                          for (const fc of msg.toolCall.functionCalls) {
                              ...
                              if (fc.name === 'changeColor') {
                                  ...
                                  if (fc.args.playing !== undefined) setIsPlaying(!!fc.args.playing);
                                  if (fc.args.volume !== undefined) setVolume(Math.max(0, Math.min(1, (fc.args.volume as number) / 100)));
                              } else if (fc.name === 'controlParticles') {
                                  if (fc.args.count) setParticleCount(Math.max(20000, Math.min(60000, fc.args.count as number)));
                                  if (fc.args.size) setParticleSize(Math.max(1, Math.min(50, fc.args.size as number)));
                                  if (fc.args.density) setModelDensity(Math.max(0, Math.min(100, fc.args.density as number)));
                              } else if (fc.name === 'controlPhysics') {
                                  if (fc.args.strength) setRepulsionStrength(fc.args.strength as number);
                                  if (fc.args.radius) setRepulsionRadius(fc.args.radius as number);
                              } else if (fc.name === 'controlView') {
                                  if (fc.args.bgMode) setBgMode(fc.args.bgMode as BackgroundMode);
                                  if (fc.args.uiHidden !== undefined) setIsUIHidden(!!fc.args.uiHidden);
                                  if (fc.args.autoRotate !== undefined) setIsAutoRotating(!!fc.args.autoRotate);
                                  if (fc.args.screensaver) setSsState('active');
                              } else if (fc.name === 'controlEffects') {
                                  if (fc.args.bloom !== undefined) setEnableBloom(!!fc.args.bloom);
                                  if (fc.args.trails !== undefined) setEnableTrails(!!fc.args.trails);
                                  if (fc.args.lyrics3D !== undefined) setUseLyricParticles(!!fc.args.lyrics3D);
                                  if (fc.args.depth !== undefined) setDepthIntensity(fc.args.depth as number);
              ...
              config: {
                  ...
                  systemInstruction: `Sen 'Partikül Yazı Morfolojisi' adındaki fütüristik web deneyiminin TAM YETKİLİ operatörüsün.
                  Kullanıcı seninle sohbet ederken sitenin GÖRÜNÜMÜNÜ, FİZİĞİNİ, SESİNİ ve EFEKTLERİNİ kontrol etmeni isteyebilir.
                  
                  Kullanabileceğin yetenekler:
                  1. 'controlMusic': Müziği durdur/başlat ve sesi 0-100 arası ayarla. ("Sesi kıs", "Müziği durdur")
                  2. 'controlParticles': Parçacık sayısını (20k-60k), boyutunu ve yoğunluğunu değiştir. ("Daha fazla nokta olsun", "Parçacıklar büyüsün")
                  3. 'controlPhysics': İtme kuvvetini ve etki alanını ayarla. ("Fareyi daha çok itsin", "Fizik motorunu güçlendir")
                  4. 'controlView': Arayüzü gizle (Temiz mod), arka planı değiştir (karanlık/aydınlık), kamerayı döndür, ekran koruyucuyu aç.
                  5. 'controlEffects': Neon parlamayı (bloom), iz efektini (trails) ve 3D yazı modunu aç/kapa.
                  6. Temel: Renk değiştir, Şekil değiştir, Efekt (preset) seç, Yazı yaz.

                  Kullanıcı Türkçe konuşursa Türkçe, İngilizce konuşursa İngilizce cevap ver.
                  Emir verildiğinde aracı SESSİZCE ve HIZLICA çalıştır, sadece kısa bir onay ver ("Yaptım", "Ayarladım", "Sesi açtım" vb.).`
              }
          });
  ...
  // *** FIXED: Updated logic to ONLY set 3D particles, NOT background ***
  const handleImageUpload = (imgSrc: string, useOriginalColors: boolean) => { 
      // Sadece 3D partikül sahnesine gönder
      handleDualImageUpload(imgSrc, null, useOriginalColors, false);
      
      // Arka plan veya deste güncellemesi YAPMA.
  };
